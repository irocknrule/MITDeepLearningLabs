...
get_ipython().system('apt-get install abcmidi timidity > /dev/null 2>&1')
...
def vectorize_string(string):
    ...
    vectorized_output = np.zeros(shape=(len(string),), dtype=np.int64)

    # Map each character in the input string to its corresponding index in the vocabulary
    for i, char in enumerate(string):
        vectorized_output[i] = char2idx[char]
...
#example_batch_loss = compute_loss(x_batch,pred)
